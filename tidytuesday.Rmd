...
results <- namesplit %>% 
  distinct(FirstName, year) %>% 
  rowwise() %>% 
  do(results = gender(namesplit$FirstName, years = namesplit$year, method = "ssa")) %>% 
  do(bind_rows(namesplit$results))

# ssa is data from social security administration from the genderdata package
